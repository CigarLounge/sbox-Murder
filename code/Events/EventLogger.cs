...
public static class EventLogger
{
	...
	[GameEvent.Round.End]
	private static void OnRoundEnd( Role winningRole, WinType winType )
	{
		if ( !Host.IsServer )
			return;

		LogEvent( EventType.Round, _timeSinceStart, $"The TODO won the round!" );
		WriteEvents();
	...
	private static void OnPlayerTookDamage( Player player )
	{
		...
		if ( attacker is Player && attacker != player )
			LogEvent( EventType.PlayerTookDamage, _timeSinceStart, $"{attacker.Client.Name} did {info.Damage} damage to {player.Client.Name}" );
		else
			LogEvent( EventType.PlayerTookDamage, _timeSinceStart, $"{player.Client.Name} took {info.Damage} damage." );
	}
	...
	private static void OnPlayerKilled( Player player )
	{
		...
		if ( player.KilledByPlayer )
			LogEvent( EventType.PlayerKill, _timeSinceStart, $"{player.LastAttacker.Client.Name} killed {player.Client.Name}" );
		else if ( player.LastDamage.Flags == DamageFlags.Fall )
			LogEvent( EventType.PlayerSuicide, _timeSinceStart, $"{player.Client.Name} fell to their death." );
	}
