...
public sealed class Inventory : IEnumerable<Carriable>
{
	...
	private readonly int[] _slotCapacity = new int[] { 1, 1 };
	...
	public void OnUse( Carriable carriable )
	{
		...
		var entities = _list.FindAll( x => x.Slot == carriable.Slot );

		if ( Active is not null && Active.Slot == carriable.Slot )
		{
	...
	public void OnChildAdded( Carriable carriable )
	{
		...
		_slotCapacity[(int)carriable.Slot] -= 1;
	}
