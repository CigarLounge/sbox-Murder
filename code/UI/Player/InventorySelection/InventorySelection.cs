...
public class InventorySelection : Panel
{
	...
	public override void Tick()
	{
		...
		var activeItemTitle = activeChild is not null ? activeChild.Title : string.Empty;
		...
		foreach ( var slot in _entries.Values )
		{
			...
			slot.SetClass( "active", slot.Carriable.IsActive );
			slot.SetClass( "opacity-heavy", slot.Carriable.IsActive );
		}
		...
		SortChildren( ( p1, p2 ) =>
		{
			...
			var result = s1.Carriable.Slot.CompareTo( s2.Carriable.Slot );
			return result != 0
				? result
				: string.Compare( s1.Carriable.Title, s2.Carriable.Title, StringComparison.Ordinal );
		} );
	...
	private void BuildInput( InputBuilder input )
	{
		...
		if ( keyboardIndexPressed != -1 )
		{
			...
			for ( var i = 0; i < childrenList.Count; ++i )
			{
				...
				{
					if ( (int)slot.Carriable.Slot == keyboardIndexPressed - 1 )
					{
